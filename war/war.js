...
$(document).ready(function() {
    ...
    function fetchSeries(n) {
        ...
        fetch(`https://api.themoviedb.org/3/discover/tv?with_genres=10768,10759&sort_by=popularity.desc&page=${n}`, options)
        .then(response => response.json())
        .then(response => {
            document.getElementById("main").innerHTML = "";
    ...
    function fetchMovies(n) {
        const options = {
            ...
            fetch(`https://api.themoviedb.org/3/discover/movie?with_genres=10752,28&sort_by=release_date.desc&release_date.lte=${today}&page=${n}`, options)
            .then(response => response.json())
            .then(response => {
