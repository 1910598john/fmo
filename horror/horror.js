...
$(document).ready(function() {
    ...
    function fetchSeries(n) {
        ...
        fetch(`https://api.themoviedb.org/3/discover/tv?with_genres=9648,10765&include_adult=false&page=${n}`, options)
        .then(response => response.json())
        .then(response => {
            document.getElementById("main").innerHTML = "";
    ...
    function fetchMovies(n) {
        const options = {
            ...
            fetch(`https://api.themoviedb.org/3/discover/movie?with_genres=27&include_adult=false&sort_by=popularity.desc&page=${n}`, options)
            .then(response => response.json())
            .then(response => {
